...
**Status:** the plan the current CI was built from. Steps 3, 4, 5 and 7 are implemented and cited by the workflows (`test.yml`, `release.yml`, `stress-nightly.yml`); read the workflows, not this file, for what gates today. Part 0 is a snapshot from 2026-08-13 and its numbers are no longer current.
