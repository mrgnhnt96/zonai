...
> test and the live test all landed, and `docs/design/oauth-internals.md` no longer lists it.
...
>
> **Items 1 and 2 are closed too** (§6). `docs/design/oauth-internals.md`'s "Still open" list
...
Picks up the third of the three items `docs/design/oauth-internals.md` lists under **Still
...
- `.game_loop/verify.yaml` still has **no rule covering `apps/web/**`**, so
  ...
  The exact YAML is at the bottom of `docs/design/admin-invite-design.md`; authorizing
...
- The other two open items in `docs/design/oauth-internals.md` are deliberately **not** in scope
...
afterwards, both proved cheaper than `docs/design/oauth-internals.md` claimed — and item 1's
...
`docs/design/oauth-internals.md` said the admin-invite suite already did this and that closing
...
- `.game_loop/verify.yaml` still has no rule for `apps/web/**`, and now none
  ...
  `docs/design/admin-invite-design.md`.
