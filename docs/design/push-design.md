...
**Status:** **built.** v1 landed on `feat/push-notifications`; the user-facing guide is [push.md](https://docs.zonai.dev/push/overview). See [What changed in the building](#16-what-changed-in-the-building) for where the code departs from the decisions below, and §15 for what the open questions turned out to be.
