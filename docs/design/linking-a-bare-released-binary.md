...
does not. `docs/design/build-fallback-next-steps.md` §2 describes what landed.
...
- No *contract* stamp is written for a released CLI, because it is built by a
  ...
  SIGABRT. See `docs/design/dart-sdk-skew.md`. The contract gap below is untouched by
  it.
...
three-row table is in `docs/design/stale-worker-guard.md`. It does not close the gap;
...
`docs/design/stale-worker-guard.md` with reasons that still hold: one CLI serves many
...
`docs/design/stale-worker-guard.md`. The remaining step is the scratch project above,
