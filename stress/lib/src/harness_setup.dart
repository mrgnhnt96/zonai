...
///
/// Repeats the `raindrop`/`zonai_schema` overrides that are already
/// committed in the fixture's own pubspec.yaml (see its comment for why
/// they're needed): a pubspec_overrides.yaml file, when present, replaces
/// pubspec.yaml's `dependency_overrides` entirely rather than merging with
/// it, so this file has to carry every override once it exists at all —
/// dropping these here silently un-fixes fixture resolution the moment the
/// harness runs, even though `dart pub get` alone (no override file yet
/// written) works fine off pubspec.yaml alone.
...
dependency_overrides:
  raindrop:
    path: ${repoRoot.path}/libs/raindrop/packages/raindrop
  zonai_schema:
    path: ${repoRoot.path}/libs/zonai_schema
  resqlite:
...
Directory _resolveRevaliRoot(Directory repoRoot) {
  ...
  // repoRoot may itself be a linked worktree (.worktrees/<name>), which sits
  // one directory deeper than the main checkout — `revali` is a sibling of
  // the main checkout, not of the worktree, so also try relative to that.
  final searchRoots = {repoRoot, ?_mainCheckoutRoot(repoRoot)};
  final candidates = [
    for (final root in searchRoots) ...[
      Directory('${root.parent.parent.path}/revali'),
      Directory('${root.parent.path}/revali'),
      Directory('${root.path}/../revali'),
    ],
  ];
...
/// Resolves the main checkout root when [repoRoot] is a linked git worktree,
/// via `git rev-parse --git-common-dir` (its parent is the main checkout).
/// Returns null outside a git repo or when repoRoot already IS the main
/// checkout (git-common-dir's parent equals repoRoot).
Directory? _mainCheckoutRoot(Directory repoRoot) {
  ...
  // git prints a path relative to repoRoot unless the common dir lives
  // outside it; resolve against repoRoot rather than this process's cwd.
  final gitCommonDir = raw.startsWith('/') ? raw : '${repoRoot.path}/$raw';
  final mainRoot = Directory(gitCommonDir).absolute.parent;
  return mainRoot.path == repoRoot.absolute.path ? null : mainRoot;
}
