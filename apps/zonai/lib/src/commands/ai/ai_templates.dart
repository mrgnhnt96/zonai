...
For reads, prefer the typed client generated by `zonai gen client`
...
both styles are valid in one file, and streams stay on `client.db.listen`.
...
project cannot be an app dependency). It generates **read** operations only
(`get`/`list`/`count`); creates, updates, deletes and `db.listen` still go
through `client.db`. Tables whose names start with `_` are zonai's own and are
skipped unless named in `client.tables.include`. Re-run it after a schema
change; `--check` fails when the committed output is stale.
...
directory the app owns; there is no default. It generates read operations only
(`get`/`list`/`count`) — writes and `db.listen` still go through `client.db` —
and skips `_`-prefixed system tables unless `client.tables.include` names them.
...
**stream**, not "realtime"/"SSE". Streams stay on `client.db.listen` even when
the project has a typed client from `zonai gen client`; that generator emits
read methods (`get`/`list`/`count`) only.
