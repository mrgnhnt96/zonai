...
Future<int> test() async {
...

  {
    logger.info('--------------------------------');
    logger.info('SIGN IN');
    final (exitCode, signedInJwt, _) = await _signIn(email);
  ...
  logger.info('PHOTOS');
  if (await _photos(jwt: jwt) case final int exitCode) {
    return exitCode;
  }
  logger.info('--------------------------------');
...
const _testImageBytes = [0xFF, 0xD8, 0xFF, 0xD9];

/// Distinct bytes used to verify PATCH replaces the on-disk image.
const _updatedImageBytes = [0xFF, 0xD8, 0xFF, 0xE0, 0x00, 0x10, 0xFF, 0xD9];
...
Future<int?> _photos({required String jwt}) async {
  ...
  final createResponse = await zonaiDB.createPhoto(
    ...
    image: Stream.value(_testImageBytes),
  );
  ...
  if (!_sameBytes(bytes.expand((e) => e).toList(), _testImageBytes)) {
    logger.error(
      'Photo view bytes mismatch: expected ${_testImageBytes.length} bytes, got ${bytes.expand((e) => e).toList().length} bytes',
    );
    return 1;
  }
  ...
  await zonaiDB.updatePhoto(
    token: jwt,
    id: photoId,
    image: Stream.value(_updatedImageBytes),
  );
  ...
  if (!_sameBytes(updatedBytes.expand((e) => e).toList(), _updatedImageBytes)) {
    logger.error(
      'Updated photo bytes mismatch: expected ${_updatedImageBytes.length} bytes, got ${updatedBytes.expand((e) => e).toList().length} bytes',
    );
    return 1;
  }
