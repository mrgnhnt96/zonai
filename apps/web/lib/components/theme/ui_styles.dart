...
List<StyleRule> get zonaiUiStyles => [
  ...
  // component's styles getter by name. A new `@css` getter is inert until that
  // file is regenerated by a full `jaspr build` — it analyzes clean, renders
  // unstyled, and gives no signal at all. `zonaiUiStyles` is already in that
  // list, so rules added here apply the moment they are written.
  css('.z-admins-panel').styles(minWidth: 280.px, raw: const {'flex-basis': '420px'}),
  css('.z-admins-list').styles(display: .flex, flexDirection: FlexDirection.column, gap: Gap.all(ZonaiSpacing.s4)),
  css('.z-admins-row').styles(
    display: .flex,
  ...
  // The disabled-control reason (design §4 item 6) and the invite form's
  // "already invited" hint. Muted, but never hidden behind a hover: the whole
  // point is that it is readable before anything is clicked.
  css('.z-admins-note').styles(
    margin: .zero,
    fontSize: 0.75.rem,
    color: mutedColor,
  ...
  css('.z-admins-invite', [
    css('&').styles(
      ...
      flexDirection: FlexDirection.column,
      gap: Gap.all(ZonaiSpacing.s4),
      padding: .only(bottom: ZonaiSpacing.s6),
      border: Border.only(
        bottom: BorderSide.solid(color: borderColor, width: 1.px),
      ),
    ),
    // `.z-field` carries a bottom margin sized for a stacked auth form; inside
