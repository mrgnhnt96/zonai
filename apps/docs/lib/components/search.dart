...
///
/// The index is ~230 KB uncompressed (~65 KB over the wire) and is only fetched
/// on first use, so it costs nothing for readers who never search.
library;
  ...
  Component _results() {
    ...
    if (_hits.isEmpty) {
      return div(classes: 'search-empty', [
        ...
        span(classes: 'search-hint', [Component.text('Live updates are documented as “stream” / “listen”.')]),
      ]);
    }

    final tokens = tokenize(_query);
    return ul(
      classes: 'search-hit-list',
      [
...
List<Component> _highlight(String source, List<String> tokens) {
  ...
  while (start < source.length) {
    ...
    parts.add(
      isMark ? Component.element(tag: 'mark', children: [Component.text(chunk)]) : Component.text(chunk),
    );
    start = end;
  }
  return parts;
...
List<StyleRule> get _styles => [
  css('.search-trigger', [
    css('&').styles(
      ...
      border: Border.all(width: 1.px, color: Color('color-mix(in srgb, currentColor 15%, transparent)')),
  ...
  css('.search-dismiss', [
    css('&').styles(
      ...
      border: Border.all(width: 1.px, color: Color('color-mix(in srgb, currentColor 18%, transparent)')),
    ),
  ...
  css('.search-empty', [
    css('&').styles(padding: Padding.all(1.5.rem), fontSize: .875.rem, opacity: .7, textAlign: TextAlign.center),
  ...
  css('.search-hit-list', [
    css('&').styles(listStyle: ListStyle.none, margin: Margin.zero, padding: Padding.zero),
  ]),
  ...
  css('.search-footer', [
    ...
    css('kbd').styles(
      ...
      border: Border.all(width: 1.px, color: Color('color-mix(in srgb, currentColor 20%, transparent)')),
      fontFamily: FontFamily.list([FontFamilies.monospace]),
    ),
  ]),
];
