...
/// OR) — with 484 sections, OR matching turns a two-word query into noise.
List<SearchHit> searchIndex(List<SearchDoc> index, String query) {
  ...
  for (final doc in index) {
    ...
    for (final section in doc.sections) {
      ...
      // Whole-phrase matches. Without these, "rate limit" ranks a deep
      // `AuthTableRateLimits` heading above the Rate Limiting overview,
      // because per-token scores alone cannot tell a page *about* the topic
      // from a page that merely mentions it.
      ...
      bestForDoc = score > bestForDoc ? score : bestForDoc;
      docHits.add(
        SearchHit(
...
final class SearchDoc {
  ...
  factory SearchDoc.fromJson(Map<String, Object?> json) => SearchDoc(
    ...
    sections: [
      for (final section in json['s']! as List) SearchSection.fromJson(section as Map<String, Object?>),
    ],
  );
