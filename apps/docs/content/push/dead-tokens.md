...
Permanent means `UNREGISTERED` or `INVALID_ARGUMENT`, and nothing else. An unrecognised status from FCM is treated as transient on purpose: guessing transient costs a retry, and guessing permanent costs a device that never hears from your app again.
