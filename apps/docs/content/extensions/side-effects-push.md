...
`platformColumn` names the column holding each row's platform, and it decides which transport carries the recipient: FCM, or APNs directly when `AppConfig.push.apns` is set. Omit it only if FCM is configured — it is the route Zonai picks for a recipient with no platform, so an APNs-only app that omits it can route nobody. See [Device Tokens](/push/device-tokens).
...
**The id is not a delivery receipt.** It says the job was written down. Nothing here, and nothing either push service offers, tells you a notification reached a phone. See [What the job id means](/push/sending#what-the-job-id-means).
...
`where` is a predicate Zonai runs and pages, so a large recipient set is never materialized in memory. The column must be declared with `$.deviceToken(...)`: the fan-out reads only the primary key, that column, and the platform column when you name one — and refuses a token column that is not a `deviceToken`. See [Device Tokens](/push/device-tokens).

## Dead tokens are Zonai's job, not yours

When the transport reports a token permanently rejected, Zonai clears it and calls `onPushRejected` first — a hook on the extension for the table the token lives on, not something you request here. A timeout is *not* a dead token and never prunes. See [Dead Tokens](/push/dead-tokens).
