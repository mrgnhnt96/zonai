...
extension SelectWithRightJoin0<S extends Schema<R>, R> on SelectFromBuilder<S, R, R> {
  ...
  SelectFromBuilder<S, R, (R?, OR)> rightJoin<OR>(
    ...
    final s = config.get(#selecting) as TableMeta<S, R>;
    final o = TableMeta.get(table)! as TableMeta<dynamic, OR>;
    ...
    return SelectFromBuilder(
      ...
      config: config.copyWith({
        ...
        #joins: <Join>[
          ...config.get(#joins) ?? [],
          RightJoin<Schema<OR>, OR>(o as TableMeta<Schema<OR>, OR>, on: on),
        ],
      }),
    );
  }
